...
const TABS = [
  { key: "home", label: "Home", href: "/", match: (p: string) => p === "/" || p === "/home" },
  { key: "explore", label: "Explore", href: "/explore", match: (p: string) => p.startsWith("/explore") },
  { key: "cart", label: "Cart", href: "/cart", match: (p: string) => p.startsWith("/cart") },
  { key: "messages", label: "Messages", href: "/messages", match: (p: string) => p.startsWith("/messages") },
  { key: "profile", label: "You", href: "/profile", match: (p: string) => p.startsWith("/profile") },
];
...
function sanitizeName(value?: string | null) {
  ...
  const v = value.trim();
  if (!v) return null;
  const ban = ["guest", "user", "new user", "unknown", "null", "undefined"];
...
function extractNameFromUser(user: any) {
  const candidates = [user?.firstName, user?.displayName, user?.name, user?.fullName, user?.username];
...
export default function Header() {
  ...
  // Scroll-driven state for the Figma-style header animation
  const [scrolled, setScrolled] = useState(false);
  const [scrollProgress, setScrollProgress] = useState(0); // 0 -> top, 1 -> fully collapsed

  // update greeting every minute
  useEffect(() => {
    const tick = () => setTimeGreeting(getTimeGreeting());
    const id = window.setInterval(tick, 60_000);
    tick();
    return () => window.clearInterval(id);
  }, []);

  // resolve greeting name from auth context or localStorage
  useEffect(() => {
    function resolve() {
      try {
        const raw = typeof window !== 'undefined' ? localStorage.getItem('unimart:user') : null;
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === 'unimart:user' || e.key === 'unimart:token') resolve();
    };
  ...
  useEffect(() => {
    ...
    window.addEventListener('unimart:cartUpdated', readCart as EventListener);
    return () => {
      window.removeEventListener('storage', readCart);
      window.removeEventListener('unimart:cartUpdated', readCart as EventListener);
    };
  }, []);

  // Listen for scroll and drive the collapse/animation progress.
  // Mirrors a Figma "scroll interaction": header compresses, greeting
  // fades/slides up, and the shadow deepens as the user scrolls down.
  useEffect(() => {
    const COLLAPSE_DISTANCE = 80; // px of scroll over which the animation completes

    function onScroll() {
      const y = window.scrollY || 0;
      const progress = Math.min(1, y / COLLAPSE_DISTANCE);
    ...
    // Set CSS variable --header-height so layout can offset content correctly
    const setHeaderHeight = () => {
      try {
        const el = headerRef.current;
        if (!el) return;
        const h = el.getBoundingClientRect().height + (typeof window !== 'undefined' ? Number(getComputedStyle(document.documentElement).getPropertyValue('--safe-area-inset-top') || 0) : 0);
  ...
  return (
    <>
      {/* Teal-Blue Mobile Header with scroll-based collapse animation */}
      <header
        role="banner"
        ref={headerRef}
        className={`fixed top-0 inset-x-0 z-50 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 transition-shadow duration-300 ${
      ...
      >
        <div className="max-w-7xl mx-auto px-3 sm:px-4">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <form onSubmit={(e) => { e.preventDefault(); router.push('/search'); }}>
                <div className={`relative flex items-center bg-gray-100 dark:bg-gray-800 rounded-full transition-all duration-200 ${
                  searchFocused ? 'ring-2 ring-teal-500/50 shadow-lg' : ''
                }`}>
                  <svg
                    className="absolute left-4 w-5 h-5 text-gray-400 dark:text-gray-500"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <input
                    ...
                    className="w-full bg-transparent py-3 pl-11 pr-4 text-base text-gray-800 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none"
                    aria-label="Search"
                  />
                </div>
              </form>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <svg viewBox="0 0 24 24" className="w-7 h-7 text-gray-700 dark:text-gray-200" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
          ...
          {/* Personalized Greeting Section — collapses/fades as the user scrolls */}
          <div
            ...
            <div className="pb-3 pt-1">
              <div className="flex items-center justify-between">
                  ...
                  >
                    <div className="w-12 h-12 rounded-full bg-teal-50 dark:bg-gray-800 flex items-center justify-center shadow-sm border-2 border-teal-100 dark:border-gray-700">
                      <span className="text-teal-600 dark:text-teal-400 font-bold text-lg">
                        {greetingName ? greetingName.charAt(0).toUpperCase() : 'U'}
                      </span>
                    </div>
                  </button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <svg viewBox="0 0 24 24" className="w-6 h-6 text-teal-600 dark:text-teal-400" fill="currentColor" strokeWidth={0}>
      ...
      <nav
        role="navigation"
        aria-label="Primary"
        className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white border-t border-gray-200 shadow-xl"
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-around h-[64px] px-1">
            {TABS.map((t) => {
              ...
              const IconComponent = getTabIcon(t.key);
              ...
              return (
                ...
                >
                  ...
                  <div className="relative mb-0.5">
                    <IconComponent className={`w-6 h-6 ${active ? 'scale-110' : ''} transition-transform duration-200`} />
...
// Logo now pulls swoop-logo.png from /public and gently scales down as the
// header collapses on scroll, echoing a Figma "smart animate" size transition.
function LinkLogo() {
  return (
